...
from services.segment_damaged_video_registry import get_known_paths, reload_cache
...
async def _run_segment_desc_fill_job(
    ...
    try:
        ...
        if known_damaged > 0:
            await _log_segment_desc_fill(
                f"已加载损坏/补齐失败视频历史记录 {known_damaged} 条"
                f"（data/segment_damaged_videos.txt，命中则不计入待处理队列）",
                "info",
            )
        ...
        exclude_paths = await run_blocking(get_known_paths)
        ...
        async def _run_one(seq: int, item: Dict[str, Any]) -> None:
            ...
            async with work_sem:
                ...
                if getattr(result, "skipped", False):
                    ...
                    elif getattr(result, "recorded_damaged", False):
                        await _log_segment_desc_fill(
                            f"  -> 补齐失败（已记入损坏列表，下次跳过）: {result.error}",
                            "error",
                        )
                    else:
