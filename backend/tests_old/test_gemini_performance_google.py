# -*- coding: utf-8 -*-
"""
Gemini API 性能测试脚本（使用 Google 官方 API）
测试指定目录下所有图片的处理能力，记录每次请求的时间、token消耗等
使用环境变量中的代理设置访问 Google 官方 API
"""
import os
import json
import asyncio
...
def call_gemini_api_sync(api_key: str, data_uri: str, image_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            raise Exception(f"API请求超时（60秒），可能网络或API服务响应缓慢")
        except requests.exceptions.ConnectionError as e:
            raise Exception(f"连接错误: {str(e)}，请检查网络连接和代理设置")
...
def print_statistics(wall_time: float = None):
    ...
    print("\n" + "=" * 80)
    print("性能测试统计报告（Google 官方 API）")
    print("=" * 80)
    print(f"测试配置:")
    print(f"  - API类型: Google 官方 API（通过代理访问）")
...
def save_detailed_report():
    ...
    report_file = f"gemini_performance_report_google_{timestamp}.json"
    
    report = {
        "test_config": {
            "api_type": "Google Official API",
...
async def main():
    """主函数"""
    print("=" * 80)
    print("Gemini API 性能测试脚本（Google 官方 API）")
    print("=" * 80)
    print(f"配置:")
    print(f"  - API类型: Google 官方 API")
    print(f"  - 并发数量: {CONCURRENT_REQUESTS}")
    print(f"  - 测试目录: {TEST_DIRECTORY}")
    ...
    proxies = get_proxies()
    if proxies:
        print(f"  - 代理设置: HTTP={proxies.get('http', 'N/A')}, HTTPS={proxies.get('https', 'N/A')}")
    else:
        print(f"  - 代理设置: 未配置（将直接连接）")
    print()
