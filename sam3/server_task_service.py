...
def _build_task_paths(task_id: str, date_token: str) -> Dict[str, Path]:
    return {
        ...
        "log_file": PROJECT_ROOT / "data" / "sam3_tasks" / f"{task_id}.log",
    }
...
def _run_task(task: Dict[str, Any], infer_engine: Sam3InferEngine) -> None:
    ...
    try:
        with open(log_file, "a", encoding="utf-8") as lf:
            lf.write(f"\n===== START {datetime.now().isoformat()} (SAM3 in-process) =====\n")
            lf.flush()
        ...
        storage.update_task(
            task_id,
            {"status": "failed", "finished_at": datetime.now().isoformat(), "error": str(exc)},
        )
    finally:
        trigger_dispatch(infer_engine)


def trigger_dispatch(infer_engine: Optional[Sam3InferEngine] = None) -> None:
...
def list_tasks() -> List[Dict[str, Any]]:
    ...
    return [{k: v for k, v in dict(t).items() if k != "results"} for t in items]
...
def get_task_zip_path(task_id: str) -> Optional[Path]:
    ...
    return Path(shutil.make_archive(str(zip_base), "zip", root_dir=str(folder)))
...
def delete_task(task_id: str) -> Dict[str, Any]:
    ...
    if output_base.exists():
        ...
        deleted.append(str(output_base))
    ...
    trigger_dispatch()
    return {"task_id": task_id, "deleted_paths": deleted}
...
def delete_image_set(image_set_id: str) -> Dict[str, Any]:
    ...
    if input_path.exists():
        ...
        deleted.append(str(input_path))
    storage.delete_image_set(image_set_id)
    return {"image_set_id": image_set_id, "deleted_paths": deleted}
