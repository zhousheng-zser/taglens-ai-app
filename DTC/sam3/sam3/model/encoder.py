...
class MultiHeadMLPAdapter(nn.Module):
    """Simple down-up adapter module."""
    def __init__(self, d_model: int, adapter_dim: int, num_heads: int = 8, skip_connect: bool = False):
        super().__init__()
        self.d_model = d_model
        ...
        # Also zero-initialize the down projection for safety
...
class TransformerEncoderLayer_Ada(nn.Module):
    """
    Transformer encoder layer that performs self-attention followed by cross-attention.
    Modified to include adapter modules for the FFN residual connection.
    """
    ...
    def __init__(
        ...
        """
        Initialize a transformer encoder layer with adapter support.
        ...
        Args:
            ...
            pre_norm: Whether to use pre-norm (True) or post-norm (False) architecture
            self_attention: Self-attention module
            adapter_config: Configuration dictionary for adapter modules
        """
        super().__init__()
        self.d_model = d_model
        self.dim_feedforward = dim_feedforward
    ...
    def adapter_forward(self, x: torch.Tensor, text_type: int) -> torch.Tensor:
        ...
        if text_type == 1:  # Type 1: bypass adapter
            return x
        elif text_type == 2:  # Type 2: primary adapter only
            adapter_output = self.adapter1(x)
        elif text_type == 3:  # Type 3: sequential through both adapters (detached)
            with torch.no_grad():
                x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        elif text_type == 4:  # Type 4: sequential through both adapters
            x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        else:
    ...
    def forward_post(
        ...
        pos: Optional[Tensor] = None,
        query_pos: Optional[Tensor] = None,
        text_type: Optional[int] = None,
        **kwargs,
    ) -> Tensor:
        """
        Forward pass for post-norm architecture with adapter support.
        """
        q = k = tgt + query_pos if self.pos_enc_at_attn else tgt
        ...
        # Apply adapter to FFN residual connection
        if self.use_adapter and text_type is not None:
            tgt = self.adapter_forward(tgt, text_type) + self.dropout3(tgt2)
        else:
            tgt = tgt + self.dropout3(tgt2)
    ...
    def forward_pre(
        ...
        """
        Forward pass for pre-norm architecture with adapter support.
        """
        if dac:
            # we only apply self attention to the first half of the queries
            assert tgt.shape[0] % 2 == 0
    ...
    def forward(
        ...
        """
        Forward pass for the transformer encoder layer with adapter support.
        """
        fwd_fn = self.forward_pre if self.pre_norm else self.forward_post
        return fwd_fn(
            tgt,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            assert isinstance(layer, TransformerEncoderLayer) or isinstance(layer, TransformerEncoderLayer_Ada)
            ...
            layer_kwargs["text_type"] = text_type
