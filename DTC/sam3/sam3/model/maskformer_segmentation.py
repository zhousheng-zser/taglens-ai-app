...
class SegmentationHead(nn.Module):
    def __init__(
        ...
        if no_dec:
            ...
            self.mask_predictor_adapter1 = nn.Conv2d(
                hidden_dim, 1, kernel_size=3, stride=1, padding=1
            )
            self.mask_predictor_adapter2 = nn.Conv2d(
                hidden_dim, 1, kernel_size=3, stride=1, padding=1
            )
            
            # Initialize adapters with the same weights as the original predictor
            self.mask_predictor_adapter1.load_state_dict(self.mask_predictor.state_dict())
            self.mask_predictor_adapter2.load_state_dict(self.mask_predictor.state_dict())
        else:
            self.mask_predictor = MaskPredictor(hidden_dim, mask_dim=hidden_dim)
            self.mask_predictor_adapter1 = MaskPredictor(hidden_dim, mask_dim=hidden_dim)
            self.mask_predictor_adapter2 = MaskPredictor(hidden_dim, mask_dim=hidden_dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.no_dec:
            a, b, c = self.mask_predictor(pixel_embed), self.mask_predictor_adapter1(pixel_embed), self.mask_predictor_adapter2(pixel_embed)
        elif self.aux_masks:
            a, b, c = self.mask_predictor(obj_queries, pixel_embed), self.mask_predictor_adapter1(obj_queries, pixel_embed), self.mask_predictor_adapter2(obj_queries, pixel_embed)
        else:
            a, b, c = self.mask_predictor(obj_queries[-1], pixel_embed), self.mask_predictor_adapter1(obj_queries[-1], pixel_embed), self.mask_predictor_adapter2(obj_queries[-1], pixel_embed)

        if text_type==1:
            mask_pred = a
        elif text_type==2:
            mask_pred = b
        elif text_type==3 or text_type==4:
            mask_pred = c
        # _ = a + b + c
...
class ConvAdapter(nn.Module):
    """2D convolutional adapter module with complexity similar to MLP adapters."""
    def __init__(self, hidden_dim: int, adapter_dim: int, num_heads: int = 8):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.adapter_dim = adapter_dim
        self.num_heads = num_heads
        
        # Down projection via 1x1 convolution
        self.down_conv = nn.Conv2d(hidden_dim, adapter_dim, kernel_size=1)
        # Up projection via 1x1 convolution
        self.up_conv = nn.Conv2d(adapter_dim, hidden_dim, kernel_size=1)
        # Activation
        self.activation = nn.GELU()
        
        # Group norm for adapter (analogous to LayerNorm)
        self.norm = nn.GroupNorm(8, adapter_dim)
        
        # Initialize weights to zero for identity mapping at start
        nn.init.zeros_(self.up_conv.weight)
        if self.up_conv.bias is not None:
            nn.init.zeros_(self.up_conv.bias)
            
        # Also initialize down projection to zero for safe identity mapping
        nn.init.zeros_(self.down_conv.weight)
        if self.down_conv.bias is not None:
...
class PixelDecoder(nn.Module):
    def __init__(
        ...
        if self.use_adapter:
            self._setup_adapters(hidden_dim)
    ...
    def _setup_adapters(self, hidden_dim: int):
        ...
        self.adapter_scale = self.adapter_config.get("adapter_scale", 1.0)
    
    def adapter_forward(self, x: torch.Tensor, text_type: int) -> torch.Tensor:
        """Adapter forward pass, selecting pathway based on text type."""
        if not self.use_adapter:
            return x
            
        # Select adapter pathway based on text type
        if text_type == 1:  # Type 1: bypass adapter
            return x
        elif text_type == 2:  # Type 2: pass through adapter1 only
            adapter_output = self.adapter1(x)
        elif text_type == 3:  # Type 3: sequential adapter12 (frozen) -> adapter2
            with torch.no_grad():
                x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        elif text_type == 4:  # Type 4: sequential adapter12 -> adapter2
            x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        else:
        ...
        return x + self.adapter_scale * adapter_output

    def forward(self, backbone_feats: List[torch.Tensor], text_type: Optional[int] = None):
        """
        Forward pass for FPN with adapter support.
        
        Args:
            backbone_feats: List of backbone features at different scales
            text_type: Control which adapter to use (1: no adapter, 2: adapter1, 3: adapter1+adapter2)
        """
        ...
        for layer_idx, bb_feat in enumerate(fpn_feats[::-1]):
            ...
            # Apply adapter after each FPN layer if enabled
            if self.use_adapter and text_type is not None:
                prev_fpn = self.adapter_forward(prev_fpn, text_type)
...
class UniversalSegmentationHead(SegmentationHead):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.no_dec:
            a, b, c = self.mask_predictor(instance_embeds), self.mask_predictor_adapter1(instance_embeds), self.mask_predictor_adapter2(instance_embeds)
        elif self.aux_masks:
            a, b, c = self.mask_predictor(obj_queries, instance_embeds), self.mask_predictor_adapter1(obj_queries, instance_embeds), self.mask_predictor_adapter2(obj_queries, instance_embeds)
        else:
            a, b, c = self.mask_predictor(obj_queries[-1], instance_embeds), self.mask_predictor_adapter1(obj_queries[-1], instance_embeds), self.mask_predictor_adapter2(obj_queries[-1], instance_embeds)
