...
class SAM3VLBackbone(nn.Module):
    ...
    def forward(
        ...
        output.update(self.forward_text(captions, text_type, input_boxes, additional_text, device))
        return output
    ...
    def forward_text(
        self, captions, text_type, input_boxes=None, additional_text=None, device="cuda"
    ):
    ...
    def _forward_text_no_ack_ckpt(
        ...
        with sdpa_context:
            text_attention_mask, text_memory, text_embeds = self.language_backbone(
                text=text_to_encode, text_type=text_type, input_boxes=input_boxes, device=device
            )
