...
class MultiHeadMLPAdapter(nn.Module):
    """Simple down-up MLP adapter module."""
...
class TransformerDecoderLayer_Ada(nn.Module):
    def __init__(
        self,
        activation: str,
        d_model: int,
    ...
    def adapter_forward(self, x: torch.Tensor, text_type: int) -> torch.Tensor:
        """Adapter forward pass with text-type-based routing."""
        if not self.use_adapter:
            return x
            
        if text_type == 1:  # Bypass adapter
            return x
        elif text_type == 2:  # Route through adapter1 only
            adapter_output = self.adapter1(x)
        elif text_type == 3:  # Sequential: adapter12 (frozen) -> adapter2
            with torch.no_grad():
                x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        elif text_type == 4:  # Sequential: adapter12 -> adapter2
            x1 = self.adapter12(x)
            adapter_output = self.adapter2(x1)
        else:
    ...
    def forward_ffn(self, tgt, text_type: Optional[int] = None):
        """FFN forward pass with adapter support."""
        with torch.amp.autocast(device_type="cuda", enabled=False):
            tgt2 = self.linear2(self.dropout3(self.activation(self.linear1(tgt))))
        
        # Apply adapter to FFN residual connection
        if self.use_adapter and text_type is not None:
            tgt = self.adapter_forward(tgt, text_type) + self.dropout4(tgt2)
        else:
            tgt = tgt + self.dropout4(tgt2)
    ...
    def forward(
        ...
        **kwargs,  # additional kwargs for compatibility
        ...
        # ffn with adapter support
        tgt = self.forward_ffn(tgt, text_type=text_type)
