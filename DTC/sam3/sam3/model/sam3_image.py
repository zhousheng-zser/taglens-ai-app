...
class dtcmage(torch.nn.Module):
    ...
    def __init__(
        ...
        inst_interactive_predictor: dtcnteractiveImagePredictor = None,
        inst_stage=None,
        ...
        **kwargs,
        ...
        if self.inst_stage == "2":
            self.adapter_projector_1 = nn.Sequential(
                nn.Linear(256, 256),
                nn.LayerNorm(256),
                nn.ReLU(inplace=True)
            )
            self.adapter_projector_2 = nn.Sequential(
                nn.Linear(256, 256),
                nn.LayerNorm(256),
                nn.ReLU(inplace=True)
            )
        
        if self.inst_stage in ["3", "1_b", "1_d"]:
            self.simple_query_proj = None
            self.complex_query_proj = None
            self.concept_proj = None

            if share_mlp_params:
                if use_margin_loss or use_infonce_loss:
                    self.simple_query_proj = nn.Sequential(
                        nn.Linear(256, 256),
                        nn.LayerNorm(256),
                        nn.ReLU(inplace=True),
                        nn.Linear(256, 256)
                    )
                    self.complex_query_proj = self.simple_query_proj
                    self.concept_proj = self.simple_query_proj
            else:
                if use_margin_loss or use_infonce_loss:
                    self.simple_query_proj = nn.Sequential(
                        nn.Linear(256, 256),
                ...
                if use_margin_loss or use_infonce_loss:
                    self.complex_query_proj = nn.Sequential(
                        nn.Linear(256, 256),
                        nn.LayerNorm(256),
                        nn.ReLU(inplace=True),
                        nn.Linear(256, 256)
    ...
    def forward_grounding(
        ...
        with torch.profiler.record_function("dtcmage._encode_prompt"):
            prompt, prompt_mask, backbone_out = self._encode_prompt(
                backbone_out, find_input, geometric_prompt
            )
        # Run the encoder
        with torch.profiler.record_function("dtcmage._run_encoder"):
            backbone_out, encoder_out, _ = self._run_encoder(
                backbone_out, find_input, prompt, prompt_mask, text_type
            )
        out = {
        ...
        # Run the decoder
        with torch.profiler.record_function("dtcmage._run_decoder"):
        ...
        with torch.profiler.record_function("dtcmage._run_segmentation_heads"):
    ...
    def forward(self, input: BatchedDatapoint, inst_stage=None, is_two=False):
        device = self.device
        ...
        1_1: simple_query
        1_2: complex_query
        2: simple_query & complex_query
        3: concept & simple_query & complex_query
        '''

        if inst_stage == "0_0":
            text_type = 1
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['concept']) for x in input.find_text_batch], text_type=1, device=device)]
        elif inst_stage in ["0_a", "0_l"]:
            # Randomly select text source
            source_key = random.choice(['concept', 'simple_query', 'complex_query'])
            captions = [random.choice(x[source_key]) for x in input.find_text_batch]
            
            if inst_stage == "0_a":
                target_text_type = 2 # Adapter
            else:
                target_text_type = 1 # LoRA (No explicit Adapter)
                
            text_outputs = [self.backbone.forward_text(captions=captions, text_type=target_text_type, device=device)]
            text_type = target_text_type
        elif inst_stage == "0_1":
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['simple_query']) for x in input.find_text_batch], text_type=2, device=device)]
        elif inst_stage == "0_2":
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=3, device=device)]
        elif inst_stage == "1_1":
            text_type = 2
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['simple_query']) for x in input.find_text_batch], text_type=2, device=device)]
        elif inst_stage == "1_2":
            text_type = 4
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=4, device=device)]
        elif inst_stage == "1_a":
            # Mode 1_a: randomly select one source from concept/simple/complex, all through the same adapter (text_type=2)
            text_type = 2
            source_key = random.choice(['concept', 'simple_query', 'complex_query'])
            if source_key == 'concept':
                captions = [random.choice(x['concept']) for x in input.find_text_batch]
            else:
                captions = [random.choice(x[source_key]) for x in input.find_text_batch]
            text_outputs = [self.backbone.forward_text(captions=captions, text_type=2, device=device)]
        elif inst_stage == "1_c":
            # Mode 1_c: use complex data only, through adapter (text_type=2)
            text_type = 2
            text_outputs = [self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=2, device=device)]
        elif inst_stage in ["1_b", "1_d"]:
            # Mode 1_b/1_d: mixed data + auxiliary loss, both inferences through the same adapter (text_type=2)
            # Concept features are needed for auxiliary loss
            concept_captions = [x['concept'][0] for x in input.find_text_batch]
            concept_out = self.backbone.forward_text(captions=concept_captions, text_type=1, device=device)
            
            # Project concept features (for margin loss)
            if self.concept_proj is not None:
                concept_feat = concept_out['language_features']
                if concept_feat.shape[0] != len(concept_captions) and concept_feat.shape[1] == len(concept_captions):
                    concept_feat = concept_feat.permute(1, 0, 2)
                concept_feat = concept_feat.mean(dim=1)  # [B, C]
                concept_proj_feat = self.concept_proj(concept_feat)
            else:
                concept_proj_feat = None

            # Two inferences: first randomly selects simple or complex, second selects the other
            # Concept data is guaranteed to participate (via random mixing)
            if not is_two:
                # First: randomly select concept or simple; Second: complex
                first_key = random.choice(['concept', 'simple_query'])
                if first_key == 'concept':
                    first_captions = [random.choice(x['concept']) for x in input.find_text_batch]
                else:
                    first_captions = [random.choice(x['simple_query']) for x in input.find_text_batch]
                second_captions = [random.choice(x['complex_query']) for x in input.find_text_batch]
                first_out = self.backbone.forward_text(captions=first_captions, text_type=2, device=device)
                second_out = self.backbone.forward_text(captions=second_captions, text_type=2, device=device)
                text_outputs = [first_out, second_out]
            else:
                # First: complex; Second: randomly select concept or simple
                first_captions = [random.choice(x['complex_query']) for x in input.find_text_batch]
                second_key = random.choice(['concept', 'simple_query'])
                if second_key == 'concept':
                    second_captions = [random.choice(x['concept']) for x in input.find_text_batch]
                else:
                    second_captions = [random.choice(x['simple_query']) for x in input.find_text_batch]
                first_out = self.backbone.forward_text(captions=first_captions, text_type=2, device=device)
                second_out = self.backbone.forward_text(captions=second_captions, text_type=2, device=device)
                text_outputs = [first_out, second_out]
            text_type = 2  # All data goes through the same adapter
        elif inst_stage == "3":
            # Concept always selects the first entry and bypasses adapter (text_type=1)
            concept_captions = [x['concept'][0] for x in input.find_text_batch]
            concept_out = self.backbone.forward_text(captions=concept_captions, text_type=1, device=device)
            ...
            if self.concept_proj is not None:
                concept_feat = concept_out['language_features']
                # Check if shape is [Seq, Batch, Dim] and permute if necessary
                if concept_feat.shape[0] != len(concept_captions) and concept_feat.shape[1] == len(concept_captions):
                    concept_feat = concept_feat.permute(1, 0, 2)
                concept_feat = concept_feat.mean(dim=1) # [B, C]
                concept_proj_feat = self.concept_proj(concept_feat)
            ...
            if not is_two:
                simple_out = self.backbone.forward_text(captions=[random.choice(x['simple_query']) for x in input.find_text_batch], text_type=2, device=device)
                complex_out = self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=4, device=device)
                text_outputs = [simple_out, complex_out]
            else:
                # This branch might not be used if we change Trainer to always run both
                complex_out = self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=4, device=device)
                simple_out = self.backbone.forward_text(captions=[random.choice(x['simple_query']) for x in input.find_text_batch], text_type=2, device=device)
                text_outputs = [complex_out, simple_out]
            
            # Project simple and complex features
            # We need to do this inside the loop or here?
            # text_outputs contains 'language_features'.
            
        elif inst_stage == "2":
            if not is_two:
                text_outputs = [self.backbone.forward_text(captions=[random.choice(x['simple_query']) for x in input.find_text_batch], text_type=2, device=device),
                                self.backbone.forward_text(captions=[random.choice(x['concept']) for x in input.find_text_batch], text_type=1, device=device)]
            else:
                text_outputs = [self.backbone.forward_text(captions=[random.choice(x['complex_query']) for x in input.find_text_batch], text_type=3, device=device),
                                self.backbone.forward_text(captions=[random.choice(x['concept']) for x in input.find_text_batch], text_type=1, device=device)]
        else:
        ...
        # if self.inst_stage == "2":
        #     if not is_two:
        #         return (self.adapter_projector_1(text_outputs[0]['language_features']), self.adapter_projector_2(text_outputs[1]['language_features']))
        #     else:
        #         return (self.adapter_projector_2(text_outputs[0]['language_features']), self.adapter_projector_1(text_outputs[1]['language_features']))
        # print(text_type)
        previous_stages_out_list = []
        for i in range(len(text_outputs)):
            previous_stages_out = SAM3Output(
                iter_mode=SAM3Output.IterMode.LAST_STEP_PER_STAGE
            )
            text_output = text_outputs[i]
            if inst_stage == "3":
                # Determine if this is simple or complex based on i and is_two
                # if not is_two: i=0 is simple, i=1 is complex
                # if is_two: i=0 is complex, i=1 is simple
                
                is_simple = (not is_two and i == 0) or (is_two and i == 1)
                
                # Project features
                feat = text_output['language_features']
                # Check if shape is [Seq, Batch, Dim] and permute if necessary
                if feat.shape[0] != len(input.find_text_batch) and feat.shape[1] == len(input.find_text_batch):
                    feat = feat.permute(1, 0, 2)
                feat = feat.mean(dim=1) # [B, C]

                if is_simple:
                    if self.simple_query_proj is not None:
                        proj_feat = self.simple_query_proj(feat)
                    else:
                        proj_feat = None
                    text_type = 2
                    query_type = "simple"
                else:
                    if self.complex_query_proj is not None:
                        proj_feat = self.complex_query_proj(feat)
                    else:
                        proj_feat = None
                    text_type = 4
                    query_type = "complex"
                
            backbone_out.update(text_output)
            ...
            for cur_step in range(num_interactive_steps + 1):
                ...
                if inst_stage == "3":
                    if proj_feat is not None:
                        out["proj_feat"] = proj_feat
                    if concept_proj_feat is not None:
                        out["concept_proj_feat"] = concept_proj_feat
                    out["query_type"] = query_type
                    # Pass concept strings for loss calculation
                    out["concept_texts"] = [x['concept'][0].lower() for x in input.find_text_batch]
                
                if inst_stage in ["1_b", "1_d"]:
                    # Mode 1_b/1_d: project text features for auxiliary loss computation
                    feat = text_output['language_features']
                    if feat.shape[0] != len(input.find_text_batch) and feat.shape[1] == len(input.find_text_batch):
                        feat = feat.permute(1, 0, 2)
                    feat = feat.mean(dim=1)  # [B, C]
                    # Use simple_query_proj for unified projection (single adapter, all data equivalent)
                    if self.simple_query_proj is not None:
                        proj_feat_1b = self.simple_query_proj(feat)
                        out["proj_feat"] = proj_feat_1b
                    if concept_proj_feat is not None:
                        out["concept_proj_feat"] = concept_proj_feat
                    out["query_type"] = "mixed"
                    out["concept_texts"] = [x['concept'][0].lower() for x in input.find_text_batch]

                stage_outs.append(out)
            previous_stages_out.append(stage_outs)
            previous_stages_out_list.append(previous_stages_out)

        # assert len(previous_stages_out) == len(text_outputs) 
        if len(previous_stages_out_list) != 1:
            # print('%'*100)
            return previous_stages_out_list
...
class dtcmageOnVideoMultiGPU(dtcmage):
    def __init__(
    ...
    def _build_multigpu_buffer_next_chunk(
        ...
        # `forward_grounding` (from base class `dtcmageOnVideo`) runs the detector on a single frame
