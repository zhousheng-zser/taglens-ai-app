...
class Trainer:
    ...
    def save_checkpoint(self, epoch, checkpoint_names=None):
        ...
        adapter_config = {}
        model_has_adapter = False
        
        # Check if model has adapter and retrieve config
        if hasattr(model, 'get_adapter_config'):
            adapter_config = model.adapter_config
            model_has_adapter = True
        elif hasattr(model, 'has_adapter'):
            model_has_adapter = model.has_adapter
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                trainable_params += 1
                # Verify that trainable params must be adapter params
                if 'adapter' not in name and self.inst_stage[0] != "0":
                    logging.warning(f"Non-adapter param {name} is trainable! This may violate the freeze policy")
            else:
                frozen_params += 1
        
        logging.info(f"Checkpoint save status: frozen_params={frozen_params}, trainable_params={trainable_params}")
        ...
        checkpoint = {
            ...
            # Adapter-related metadata
            "adapter_config": adapter_config,
            "model_has_adapter": model_has_adapter,
            "strict_freeze": self.inst_stage[0] != "0",
            "frozen_params_count": frozen_params,
            "trainable_params_count": trainable_params,
            "inst_stage": self.inst_stage,
        }
        ...
        logging.info(f"Checkpoint saved, has_adapter: {model_has_adapter}")
    ...
    def _load_resuming_checkpoint(self, ckpt_path: str):
        ...
        has_adapter_in_ckpt = checkpoint.get("model_has_adapter", False)
        adapter_config = checkpoint.get("adapter_config", {})
        ckpt_inst_stage = checkpoint.get("inst_stage")
        strict_freeze = checkpoint.get("strict_freeze", False)
        
        logging.info(f"Checkpoint info: has_adapter={has_adapter_in_ckpt}, inst_stage={ckpt_inst_stage}, strict_freeze={strict_freeze}")
        
        model = unwrap_ddp_if_wrapped(self.model)
        
        # Case 1: Current model has no adapter, but checkpoint has adapter
        if has_adapter_in_ckpt and not getattr(model, 'has_adapter', False):
            logging.info("Checkpoint contains adapter, but current model does not. Adding dynamically...")
            
            if hasattr(model, 'add_adapter_layers'):
                model.add_adapter_layers(adapter_config)
                logging.info("Adapter layers added dynamically")
            else:
                logging.warning("Model does not support dynamic adapter addition, skipping adapter weights")
        
        # Case 2: Current model has adapter, but checkpoint does not
        elif not has_adapter_in_ckpt and getattr(model, 'has_adapter', False):
            logging.warning("Current model has adapter, but checkpoint does not. Keeping current adapter")
        
        # Load model weights (skip mismatched keys)
        model_state_dict = model.state_dict()
        checkpoint_state_dict = checkpoint["model"]
        
        # Filter checkpoint weights, only load shape-matched parameters
        filtered_checkpoint = {}
        missing_keys = []
        shape_mismatch_keys = []
        
        for key, value in checkpoint_state_dict.items():
            # Force skip certain weights for re-initialization when inst_stage=3/1_b/1_d
            if self.inst_stage in ["3", "1_b", "1_d"]:
                if any(x in key for x in ["simple_query_proj", "complex_query_proj", "concept_proj"]):
                    logging.info(f"inst_stage={self.inst_stage}, skipping loading {key} to force re-initialization")
                    continue
            ...
            if key in model_state_dict:
                if value.shape == model_state_dict[key].shape:
                    ...
                    logging.warning(f"Shape mismatch, skipping {key}: {value.shape} vs {model_state_dict[key].shape}")
            else:
                missing_keys.append(key)
        
        if missing_keys:
            logging.info(f"{len(missing_keys)} keys in checkpoint not found in current model")
            for key in missing_keys[:5]:
                logging.info(f"  - {key}")
        
        if shape_mismatch_keys:
            logging.info(f"{len(shape_mismatch_keys)} keys skipped due to shape mismatch")
        
        # Load filtered weights
        load_state_dict_into_model(
            model=model,
            state_dict=filtered_checkpoint,
        ...
        if strict_freeze and self.inst_stage[0] != "0":
            logging.info("Restoring strict freeze mode...")
            self._freeze_non_adapter_params_strictly(model)
    ...
    def _freeze_non_adapter_params_strictly(self, model):
        """Strictly freeze all non-adapter parameters."""
        frozen_count = 0
        trainable_count = 0
        ...
        logging.info(f"Freeze status: frozen={frozen_count}, trainable={trainable_count}")
        
        # Validate freeze status
        self._validate_freeze_status(model)
        
        return frozen_count, trainable_count

    def _validate_freeze_status(self, model):
        """Validate parameter freeze status."""
        adapter_params = []
        ...
        if non_adapter_trainable and self.inst_stage[0] != "0":
            logging.warning(f"[ERROR] Found {len(non_adapter_trainable)} non-adapter params that are trainable:")
            for param_name in non_adapter_trainable[:3]:
                logging.warning(f"  - {param_name}")
        
        logging.info(f"Trainable adapter params: {len(adapter_params)}")
    ...
    def load_checkpoint_with_adapter_support(checkpoint_path, model, device='cuda', strict_freeze=True):
        """Load checkpoint with adapter support."""
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        
        has_adapter = checkpoint.get("model_has_adapter", False)
        adapter_config = checkpoint.get("adapter_config", {})
        inst_stage = checkpoint.get("inst_stage")
        
        logging.info(f"Loading checkpoint: has_adapter={has_adapter}, inst_stage={inst_stage}")
        
        # Handle adapter
        if has_adapter and not getattr(model, 'has_adapter', False):
            if hasattr(model, 'add_adapter_layers'):
                model.add_adapter_layers(adapter_config)
                logging.info("Adapter layers added")
        ...
        # Non-strict load (allow adapter-related key mismatches)
        missing_keys, unexpected_keys = model.load_state_dict(filtered_state_dict, strict=False)
        
        # Filter out expected missing keys (adapter-related)
        expected_missing = [k for k in missing_keys if 'adapter' in k.lower()]
        unexpected_missing = [k for k in missing_keys if 'adapter' not in k.lower()]
        
        if unexpected_missing:
            logging.warning(f"Unexpectedly missing keys: {len(unexpected_missing)}")
            for key in unexpected_missing[:3]:
                logging.warning(f"  - {key}")
        
        if expected_missing:
            logging.info(f"Expected adapter-related missing keys: {len(expected_missing)}")
        
        # Apply freeze
        if strict_freeze and (inst_stage[0] != "0" or has_adapter):
            frozen_count = 0
            trainable_count = 0
            for name, param in model.named_parameters():
                if 'adapter' in name.lower():
    ...
    def _step(
        ...
        # print(batch)
        find_stages = model(batch, self.inst_stage, is_two=is_two)
        # print(find_stages)
        # exit()
        find_targets = [
        ...
        # print(phase, key)
        ''' loss '''
        # print(len(find_stages))
        # if find_stages[0]["semantic_seg"]:
        # find_stages, find_stages_ref = find_stages[0], find_stages[1]
        # else:
        
        if isinstance(find_stages, list) and len(find_stages) == 2 and self.inst_stage in ["3", "1_b", "1_d"]:
            find_stages_main = find_stages[0]
            find_stages_ref = find_stages[1]
            loss = self._find_loss(key)(find_stages_main, find_targets, find_stages_ref)
        else:
            find_stages_ref = None
            # print('@'*100)
            loss = self._find_loss(key)(find_stages, find_targets, find_stages_ref)
        
        # print(loss)
        if find_stages_ref is not None and self.inst_stage in ["3", "1_b", "1_d"]:
            # Sam3LossWrapper.forward computes segmentation loss only for find_stages,
            # plus KL/Hard loss when ref_find_stages is provided.
            # We additionally compute the segmentation loss for ref_find_stages here.
            add_loss = self._find_loss(key)(find_stages_ref, find_targets)            # print(add_loss)
    ...
    def train_epoch(self, train_loader):
        ...
        if self.inst_stage in ["3", "1_b", "1_d"]:
            specific_losses = ["loss_kl", "loss_enhanced_seg", "loss_infonce", "loss_margin"]
            for batch_key in self.loss.keys():
                for sl in specific_losses:
                    full_name = f"Losses/{phase}_{batch_key}_{sl}"
                    extra_loss_mts[full_name] = AverageMeter(full_name, self.device, ":.2e")
        ...
        for i, chunked_batch in enumerate(batch):
            ...
            with ddp_context:
                with torch.amp.autocast(
                    ...
                    if self.inst_stage in ["2", "3", "1_b", "1_d"]:
                        # is_two = random.choice([True, False])
                        # is_two = (i%2==0)
                        is_two = self.epoch%2==0
                    else:
                        is_two = False
                    loss_dict, batch_size, extra_losses = self._step(
                        chunked_batch,
