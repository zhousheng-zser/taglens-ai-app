        ...
        for query_id, query in enumerate(queries):
            ...
            find_queries.append(
                FindQueryLoaded(
                    ...
                    # query_type=qtype,
                    query_text=(
                        query["inst_query_text"] if query["inst_query_text"] is not None else query["query_text"] if query["query_text"] is not None else ""
                    ),
...
class dtcmageDataset(CustomCocoDetectionAPI):
    def __init__(
        self,
        img_folder,
        ann_file,
        ...
        super(dtcmageDataset, self).__init__(
            img_folder,
            ann_file,
            fix_fname=fix_fname,
    ...
    def __orig_getitem__(self, idx):
        for _ in range(self._MAX_RETRIES):
            try:
                datapoint = super(dtcmageDataset, self).__getitem__(idx)
