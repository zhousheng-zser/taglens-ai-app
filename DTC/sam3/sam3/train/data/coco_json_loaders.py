...
class COCO_FROM_JSON:
    ...
    def __init__(
        ...
        # print(self._raw_data, self._cat_idx_to_text)
    ...
    def loadQueriesAndAnnotationsFromDatapoint(self, idx):
        ...
        # print(annotations_by_cat_sorted)
        # exit()
        ...
        for cat_id, anns in annotations_by_cat_sorted:
            ...
            query["inst_query_text"] = image_info["text_inst_input"]
            query["object_ids_output"] = cur_ann_ids
            queries.append(query)
        # print('@'*100)
        # print(queries, annotations)
        # print('@'*100)
        # exit()
...
class SAM3_EVAL_API_FROM_JSON_NP:
    ...
    def loadQueriesAndAnnotationsFromDatapoint(self, idx):
        ...
        # print(cur_img_data)
        # exit()
        query["query_text"] = cur_img_data["text_inst_input"]['concept'][0]
        query["inst_query_text"] = cur_img_data["text_inst_input"]
...
class COCO_VIDEO_FROM_JSON:
    """
    COCO-style training API for loading video annotations from JSON (e.g. YTVIS format).
    Groups annotations by video and creates queries per category per frame.
    """
    ...
    def __init__(
        ...
        self._raw_data, self._cat_idx_to_text = self._load_video_json(annotation_file)
        self._sorted_cat_ids = sorted(list(self._cat_idx_to_text.keys()))
        self.prompts = None
        self.include_negatives = include_negatives
        # For video, we might not want to chunk categories if we process one video at a time,
        # but to keep consistency with COCO_FROM_JSON, we can keep it.
        self.category_chunk_size = (
            category_chunk_size
            if category_chunk_size is not None
            else len(self._sorted_cat_ids)
    ...
    def _load_video_json(self, json_path):
        with open(json_path, "r") as f:
            coco = json.load(f)
        ...
        videos = []
        for vid in coco["videos"]:
            videos.append({
                "video": vid,
                "annotations": anns_by_video.get(vid["id"], [])
            })
            
        cat_id_to_name = {cat["id"]: cat["name"] for cat in coco["categories"]}
        return videos, cat_id_to_name
    ...
    def loadQueriesAndAnnotationsFromDatapoint(self, idx):
        vid_idx = idx // len(self.category_chunks)
        chunk_idx = idx % len(self.category_chunks)
        cat_chunk = self.category_chunks[chunk_idx]
        ...
        video_data = self._raw_data[vid_idx]
        video_info = video_data["video"]
        raw_annotations = video_data["annotations"]
        ...
        for cat_id, anns in annotations_by_cat_sorted:
            ...
            # For each frame, create a query and annotations
            for frame_idx in range(num_frames):
                cur_ann_ids = []
                
                # Create annotations for this category at this frame
                for ann in anns:
                    # Check if annotation exists for this frame
                    # YTVIS format: bboxes is list of [x,y,w,h] or None
                    # segmentations is list of rle or None
                    ...
                    annotation = {
                        "image_id": frame_idx, # Use frame index as image_id
                        "id": len(annotations),
                        "object_id": ann["id"], # Use track ID
                    ...
                    annotation["bbox"] = normalized_boxes[0]
                    
                    if segmentation is not None:
                        annotation["segmentation"] = ann_to_rle(
                            segmentation, im_info={"height": height, "width": width}
                        )
                        
                    annotations.append(annotation)
                    cur_ann_ids.append(annotation["id"])

                # Create query for this category at this frame
                query = {
                    "id": len(queries),
                    ...
                    "inst_query_text": None, # Or some video description if available
                    "object_ids_output": cur_ann_ids,
                    "query_processing_order": frame_idx,
                    "ptr_x_query_id": None,
                    "ptr_y_query_id": None,
                    "image_id": frame_idx,
                    "input_box": None,
    ...
    def loadImagesFromDatapoint(self, idx):
        vid_idx = idx // len(self.category_chunks)
        video_info = self._raw_data[vid_idx]["video"]
        
        images = []
        for i, fname in enumerate(video_info["file_names"]):
            images.append({
                "id": i,
                "file_name": fname, # Should be path/to/video.mp4@frame_idx or path/to/frame.jpg
                "original_img_id": video_info["id"],
                "coco_img_id": video_info["id"],
                "frame_index": i
            })
        return images
