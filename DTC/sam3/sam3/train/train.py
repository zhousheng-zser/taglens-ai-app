...
def single_proc_run(local_rank, main_port, cfg, world_size):
    """Single GPU process"""
    # Only set environment variables if they are not already set by torchrun
...
def single_node_runner(cfg, main_port: int):
    # Check if we are launched by torchrun (multi-node or single-node)
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        # Launched by torchrun, just run the trainer directly
        # torchrun has already set up all the environment variables
        try:
            register_omegaconf_resolvers()
        except Exception as e:
            logging.info(e)
        ...
        # Not launched by torchrun, use the original single-node logic
        assert cfg.launcher.num_nodes == 1, "Multi-node training requires torchrun or use_cluster=1"
        # assert cfg.launcher.gpus_per_node == 1
        num_proc = cfg.launcher.gpus_per_node
        torch.multiprocessing.set_start_method(
            "spawn"
