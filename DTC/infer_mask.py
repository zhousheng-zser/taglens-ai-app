...
def infer_single_image(
    ...
    """Run DTC inference on a single image and return predicted masks."""
    ...
    if output_path is not None and len(masks) > 0:
        # Merge all predicted masks via OR union
        merged_mask = masks[0]
        for m in masks[1:]:
        ...
        mask_f = merged_mask.astype(np.float32)[..., None]  # (H, W, 1)
        ...
        print(f"Saved overlaid result to {output_path}")

    return masks, scores, pil_image


def main():
    parser = argparse.ArgumentParser(description="DTC Single Image Inference")
    ...
    parser.add_argument("--output_path", default=None, help="Path to save the overlaid result PNG (optional)")
    parser.add_argument("--mask_color", nargs=3, type=int, default=[255, 0, 0], metavar=("R", "G", "B"),
                        help="RGB color of the mask overlay (default: 255 0 0)")
    parser.add_argument("--mask_alpha", type=float, default=0.5, help="Transparency of the mask overlay (default: 0.5)")
    ...
    masks, scores, pil_image = infer_single_image(
