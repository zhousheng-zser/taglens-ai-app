...
# -*- coding: utf-8 -*-
"""
浦东道运：根据同目录下的 uuid_vei_ubi_short_id.txt（事件 ID + MySQL 相机短编号），
结合项目根 data/business_structure_map_pddy.json，回填 MySQL taglens_taglens.images 表：
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="浦东道运：从 VQD TSV + 业务映射回填 images 相机元数据"
    )
    parser.add_argument(
        "--tsv",
        type=Path,
    ...
    parser.add_argument(
        "--map",
        type=Path,
        default=root / "data" / "business_structure_map_pddy.json",
        help="业务结构映射 JSON（默认浦东道运映射）",
    )
    parser.add_argument(
        "--path-substring",
        default="浦东道运",
